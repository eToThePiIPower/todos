...
RSpec.describe TodoListsController, type: :controller do
  ...
  describe 'GET #show' do
    ...
    context 'when the owner is logged in' do
      ...
      it 'assigns a new item for the add item form' do
        sign_in @owner

        get :show, id: @todo_list

        expect(assigns(:todo_item)).to be_a_new(TodoItem)
          .with(todo_list_id: @todo_list.id)
      end
    end
  ...
  describe 'GET #new' do
    context 'when a user is signed in' do
      it 'returns a form for a new list' do
        @user = create(:user)
        sign_in @user
        get :new

        expect(assigns[:todo_list]).to be_a_new(TodoList).with(user_id: @user.id)
      end
    end
  ...
  describe 'GET #edit' do
    before do
      @owner = create(:user)
      @todo_list = create(:todo_list, user_id: @owner.id)
    end
    ...
    context 'when nobody is logged in with an error' do
    ...
    context 'when the owner is logged in' do
      ...
      context 'with valid params' do
        it 'redirects to the newly updated todo list' do
          ...
          expect(response.status).to be 303 # See Other
